...
class GUI_FaxReceive():
    ...
    def _load_config_file(self):
        ...
        self.current_folder = self.fax_root_folder
    ...
    def _create_widgets(self):
        ...
        # Button(self.frame_right, width=13,  height=3, text="즐겨찾기" , command=self._handler_btn_bookmark).grid(row=5 ,column=2,padx= 10 ,pady= 2,sticky='nsew')
    ...
    def _run_with_viewer(self):  
        ...
        if  selected_file : 
            for src_name in selected_file:
                ...
                self.fax_control.run_with_viewer(self.current_folder,  src_name)
        else :
    ...
    def _handler_btn_fax_folder_open(self):
        """
        선택한 폴더명의 디렉토리를 리스트 박스 창에 띄우기
        ...
        if entry_value:
            import lib_move_file
            a = lib_move_file.MoveFaxFile()
            folder_path = a.search_folder_by_keyword(entry_value.split('_')[0])
            print(f'{folder_path=}')
            if folder_path :
                file_list = self.fax_control.get_sorted_filename(folder_path)
                self.file_listbox.delete(0,END)
    ...
    def _handler_btn_bookmark(self):
        pass
